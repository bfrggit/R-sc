...
# create plot objects
plot_obj_int <- ggplot(data = sr_int_proc, aes(x = prob)) + ggplot_theme +
    ...
    geom_errorbar(
        ...
        width = (max(sr_int_proc$prob) - min(sr_int_proc$prob)) * 0.02
    ) + geom_point(
...
plot_obj_cal <- ggplot(data = sr_cal_proc, aes(x = prob)) + ggplot_theme +
    xlab("Probability of sensor presence") +
    ...
    geom_errorbar(
        ...
        width = (max(sr_cal_proc$prob) - min(sr_cal_proc$prob)) * 0.02
    ) + geom_point(
        aes(y = value[, "mean"], color = selector, shape = selector), size = 2
    ) + scale_color_selector + scale_shape_selector +
    guides(color = guide_legend(nrow = 1, title.position = "left"))
...
ggsave(
    filename = "t_plot/no_move_prob_iter.pdf",
    plot = plot_obj_int,
    device = cairo_pdf,
    width = 8,
    height = 5,
...
ggsave(
    filename = "t_plot/no_move_prob_cali.pdf",
    plot = plot_obj_cal,
    device = cairo_pdf,
    width = 8,
    height = 5,
