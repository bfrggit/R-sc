...
print_usage() {
    echo >&2 "Usage: $0 NUM_NODES SELECTOR CASE_GRAPH CASE_LOC PROJ_ROOT OUTPUT_FILE"
}
...
sensor_fn=$(realpath "scenarios/types_cs3.RData")
if [ $? -ne 0 ]; then exit $?; fi
...
presence_fn=$(realpath "prep_p_cs3_v_nodes_RData/presence_${num_nodes}_${PROB}_${case_presence}.RData")
if [ $? -ne 0 ]; then exit $?; fi
