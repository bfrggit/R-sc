...
get_multi_paths_ompr_gurobi <<- function(
    ...
    which_effective <- c(1L, which_selected)
    num_selected <- length(which_selected)
    num_spots_effective <- 1L + num_selected
    distance_effective <- distance_matrix[which_effective, which_effective]
    if(is.null(num_workers)) { num_workers <- num_selected }
    ...
    add_variable(
        x[i, j, k],
        i = 1L:num_spots_effective,
        j = 1L:num_spots_effective,
        k = 1L:num_workers,
        type = "binary"
    ) %>%
    ...
    add_variable(
        ...
        k = 1L:num_workers,
        lb = 1,
        ub = num_spots_effective
    ) %>%

    # minimize travel distance and latest arrival
    set_objective(
        sum_expr(
            distance_effective[i, j] * x[i, j, k],
            i = 1L:num_spots_effective,
            j = 1L:num_spots_effective,
            k = 1L:num_workers
        ), "min"
    ...
    add_constraint(
        sum_expr(
            x[1L, j, k],
            j = 1L:num_spots_effective # j = 2L:NUM_SPOTS
        ) == 1,
        k = 1L:num_workers,
        .show_progress_bar = FALSE
    ) %>%
    ...
    add_constraint(
        sum_expr(
            x[i, 1, k],
            i = 1L:num_spots_effective # i = 2L:NUM_SPOTS
        ) == 1,
        k = 1L:num_workers,
        .show_progress_bar = FALSE
    ) %>%
    ...
    add_constraint(
        u[i, k] - u[j, k] + 1 <= (num_spots_effective - 1) * (1 - x[i, j, k]),
        i = 2L:num_spots_effective,
        j = 2L:num_spots_effective,
        k = 1L:num_workers,
        .show_progress_bar = FALSE
    )
    ...
    if(is.finite(max_cost_worker)) {
        model <- add_constraint(
            .model = model,
            sum_expr(
                x[i, j, k] * (distance_effective[i, j] + spot_cali_cost[j]),
                i = 1L:num_spots_effective,
                j = 1L:num_spots_effective
            ) <= max_cost_worker,
            k = 1L:num_workers,
    ...
    for(jnd in 1L:nrow(solution)) {
        ...
        paths_array[
            which_effective[solution$i[jnd]],
            which_effective[solution$j[jnd]],
            worker_id
