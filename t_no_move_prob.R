...
PROB_ALL <- seq(5L, 100L, by = 5L)
SELECTORS <- c("minimal", "all", "nodal")
lockBinding("PROB_ALL", globalenv())
...
colnames(simu_res_mat) <-
    c("selector", "metric", "prob", "case_type", "case_node", "value")
    ...
    for(prob in PROB_ALL) {
        for(jnd_pres in 1L:NUM_CASES_USED_NODES) {
            ...
            for(sel_name in SELECTORS) {
                ...
                # record results
                simu_res[jnd_res, ] <- list(
                    selector    = sel_name,
                    metric      = "interval_mean",
                    prob        = prob,
                    case_type   = jnd_types,
                    case_node   = jnd_pres,
                    value       = 1e+7 / sum(res_case$intervals)
                )
                jnd_res <- jnd_res + 1L
...
cat("\n")
cat("Done!\n")
save.image(file = "t_data/no_move_prob.RData")
